...
Use timeout to control client-side HTTP deadline, or "none" for no timeout.`,
    inputSchema: z.object({
      ...
      select: z
        ...
        .optional()
        .describe(
          'Cherry-pick step result(s) by step ID. Single: "synthesise". Multiple: "step_a,step_b". Takes priority over return.',
        ),
      return: z
        .enum(["result", "full", "steps", "trace"])
        .optional()
        .describe(
          '(run only) Response shape. "result" (default) = last step only. "full" = everything. "steps" = all results. "trace" = trace only.',
        ),
      ...
      timeout: z
        ...
        .optional()
        .describe(
          'Client-side HTTP timeout in seconds. Default: 300 (5 min) for run/resume, 15 for others. "none" disables timeout entirely.',
        ),
    }),
  },
  async (params) => {
