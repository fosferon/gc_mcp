...
plan is a deprecated compatibility alias for critical_path; it is not the gc_plan scheduler.`,
    inputSchema: z.object({
      ...
      detail: z
        ...
        .optional()
        .describe(
          "Projection for query/list/search/ready/show/tree/critical_path/traverse. minimal = exactly id/title/status/priority/labels/blocked_by and omits project/timestamps; compact adds coordination fields and timestamps but omits description; full returns the complete issue. Defaults to compact except show, which defaults to full for compatibility.",
        ),
      intent: z
        .string()
        .optional()
      ...
      mode: z
        ...
        .optional()
        .describe(
          "For action=list: `flat` (default) pages individual issues; `tree` pages root issues while retaining each root's complete dependency subtree.",
        ),
      tree: zBoolean()
        .optional()
...
Start with 'coverage' to see how far back mail has actually been collected before trusting a query's completeness.`,
    inputSchema: z.object({
      ...
      smtp_ssl: zBoolean()
        .optional()
        .describe(
          "Implicit TLS (SMTPS). Defaults from smtp_port on add_endpoint — 465 is true, " +
